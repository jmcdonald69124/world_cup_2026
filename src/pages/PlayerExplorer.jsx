...
function PlayerCard({ player, onSelect, onCompare, compareSelected }) {
  ...
  return (
    <motion.div layout whileHover={{ scale: 1.02, y: -2 }} className={`glass-card rounded-xl p-4 cursor-pointer transition-all ${isCompareSelected ? 'border-wcGold/50 bg-wcGold/5' : 'hover:border-white/20'}`} onClick={() => onSelect(player)}>
      ...
      <button onClick={(e) => { e.stopPropagation(); onCompare(player); }} className={`w-full py-1.5 rounded-lg text-xs font-semibold transition-all ${isCompareSelected ? 'bg-wcGold/20 text-wcGold border border-wcGold/30' : 'bg-white/5 text-gray-400 hover:text-white hover:bg-white/10'}`}>
    ...
    <div className="min-h-screen pt-20 pb-12 px-4">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex flex-wrap gap-3 mb-5">
          <input type="text" placeholder="Search players or teams..." value={search} onChange={(e) => setSearch(e.target.value)} className="flex-1 min-w-48 bg-white/5 border border-white/10 rounded-lg px-4 py-2 text-sm text-white placeholder-gray-600 focus:outline-none focus:border-wcGreen/50" />
          <div className="flex gap-2">
