...
export default function OddsExplainer({ homeTeam = 'Argentina', awayTeam = 'France', initialOdds }) {
  ...
  const outcomes = [
    { key: 'home', label: homeTeam, odds: homeOdds, setOdds: setHomeOdds },
    { key: 'draw', label: 'Draw', odds: drawOdds, setOdds: setDrawOdds },
    { key: 'away', label: awayTeam, odds: awayOdds, setOdds: setAwayOdds },
  ];
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-3 gap-3">
        {outcomes.map((outcome) => {
          ...
          return (
            <motion.div key={outcome.key} whileHover={{ scale: 1.02 }} onClick={() => setSelectedOutcome(outcome.key)}
              className={`glass-card rounded-xl p-4 cursor-pointer transition-all ${isSelected ? 'border-wcGold/40 bg-wcGold/5' : 'hover:border-white/20'}`}>
              ...
              <div className="space-y-0.5">
                <div className="text-xs text-gray-500"><span className="text-gray-400">American:</span> {american}</div>
                <div className="text-xs text-gray-500"><span className="text-gray-400">Fractional:</span> {fractional}</div>
                <div className="text-xs text-wcGold font-medium">{implied}% implied</div>
              </div>
              <input type="range" min="1.1" max="10" step="0.05" value={outcome.odds}
      ...
      <div className="glass-card rounded-xl p-4">
        ...
        <div className="space-y-2">
          {outcomes.map((outcome) => {
            ...
            return (
              <div key={outcome.key}>
                <div className="flex justify-between text-xs mb-1"><span className="text-gray-400">{outcome.label}</span><span className="text-white font-medium">{implied}%</span></div>
                <div className="h-2 bg-gray-800 rounded-full overflow-hidden">
                  <motion.div className="h-full bg-wcGold rounded-full" initial={{ width: 0 }} animate={{ width: `${Math.min(implied, 100)}%` }} transition={{ duration: 0.5 }} />
                </div>
              </div>
            );
          })}
        ...
        <div className="mt-3 pt-3 border-t border-white/5 flex justify-between items-center">
          ...
          <span className={`text-sm font-bold ${parseFloat(margin) > 0 ? 'text-wcRed' : 'text-wcGreen'}`}>{(100 + parseFloat(margin)).toFixed(1)}%</span>
        </div>
      </div>
      <div className="glass-card rounded-xl p-4 border border-wcRed/20 bg-wcRed/5">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-gray-400 mt-1">The extra percentage bookmakers charge. This is how they guarantee profit regardless of outcome.</p>
          </div>
      ...
      <div className="glass-card rounded-xl p-4">
        ...
        <div className="flex gap-3 mb-4">
          <div className="flex-1">
            ...
            <input type="number" value={stake} onChange={(e) => setStake(Math.max(1, parseInt(e.target.value) || 1))}
        ...
        <div className="bg-white/5 rounded-lg p-3 space-y-2">
          <div className="flex justify-between text-sm"><span className="text-gray-400">Odds</span><span className="text-white font-semibold">{selectedOdds.toFixed(2)}</span></div>
          <div className="flex justify-between text-sm"><span className="text-gray-400">Implied probability</span><span className="text-white font-semibold">{impliedProb}%</span></div>
          <div className="flex justify-between text-sm"><span className="text-gray-400">Potential profit</span><span className="text-wcGreen font-bold">+${profit}</span></div>
          <div className="flex justify-between text-sm border-t border-white/10 pt-2"><span className="text-gray-400">Total return</span><span className="text-white font-bold">${totalReturn}</span></div>
        </div>
        <p className="text-xs text-gray-600 mt-2">If you bet ${stake} on {selectedOutcome === 'home' ? homeTeam : selectedOutcome === 'away' ? awayTeam : 'Draw'} at {selectedOdds.toFixed(2)} odds, you would win ${profit} profit (total return: ${totalReturn}).</p>
      </div>
    </div>
  );
}
