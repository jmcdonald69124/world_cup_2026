...
export default function WinProbabilityGauge({ homeProb, drawProb, awayProb, homeTeam, awayTeam }) {
  ...
  useEffect(() => {
    ...
    [0, 0.25, 0.5, 0.75, 1.0].forEach((t) => {
      ...
      svg.append('line').attr('x1', cx + (outerRadius + 5) * Math.sin(angle)).attr('y1', cy - (outerRadius + 5) * Math.cos(angle)).attr('x2', cx + (outerRadius + 12) * Math.sin(angle)).attr('y2', cy - (outerRadius + 12) * Math.cos(angle)).attr('stroke', '#374151').attr('stroke-width', 1.5);
    });
    ...
    needleGroup.append('line').attr('x1', 0).attr('y1', 0).attr('stroke', '#FFD700').attr('stroke-width', 3).attr('stroke-linecap', 'round').attr('x2', needleRadius * Math.sin(startAngle)).attr('y2', -needleRadius * Math.cos(startAngle)).transition().duration(1000).ease(d3.easeBounceOut).attr('x2', needleX - cx).attr('y2', needleY - cy);
