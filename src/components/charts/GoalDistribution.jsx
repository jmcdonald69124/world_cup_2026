...
export default function GoalDistribution({ data }) {
  ...
  useEffect(() => {
    ...
    g.selectAll('.bar').data(chartData).enter().append('rect').attr('class', 'bar').attr('x', d => xScale(d.minute)).attr('y', innerHeight).attr('width', xScale.bandwidth()).attr('height', 0).attr('fill', 'url(#goalBarGradient)').attr('rx', 2).style('cursor', 'pointer')
      ...
      .transition().duration(800).delay((_, i) => i * 30).ease(d3.easeBackOut.overshoot(0.5)).attr('y', d => yScale(d.count)).attr('height', d => innerHeight - yScale(d.count));
