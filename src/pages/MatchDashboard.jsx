          ...
          {standings.map((team, i) => (
            <tr key={team.id} className={`border-t border-white/5 ${i < 2 ? 'text-white' : 'text-gray-500'}`}>
              <td className="py-1 flex items-center gap-1.5">
                ...
                {i < 2 && <span className="w-1 h-1 rounded-full bg-wcGreen" />}
              </td>
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex flex-wrap gap-1.5 mb-5">
              {GROUPS.map((g) => (
                <button
                  key={g}
                  onClick={() => setActiveFilter(activeFilter === `Group ${g}` ? 'All' : `Group ${g}`)}
                  className={`px-2.5 py-1 rounded-md text-xs font-bold transition-all ${
                    activeFilter === `Group ${g}`
