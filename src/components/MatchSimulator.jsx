    ...
    <div className="space-y-6">
      <div className="grid grid-cols-2 gap-3">
        <div className="bg-white/5 rounded-xl p-4 text-center">
          ...
          <motion.div key={homeXG.toFixed(2)} initial={{ opacity: 0.4, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="text-3xl font-black text-wcGreen tabular-nums">{homeXG.toFixed(2)}</motion.div>
          <div className="text-xs text-gray-600 mt-1">expected goals</div>
        </div>
        <div className="bg-white/5 rounded-xl p-4 text-center">
          <div className="text-xs text-gray-500 mb-1">{awayTeam} xG</div>
          <motion.div key={awayXG.toFixed(2)} initial={{ opacity: 0.4, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="text-3xl font-black text-wcRed tabular-nums">{awayXG.toFixed(2)}</motion.div>
          <div className="text-xs text-gray-600 mt-1">expected goals</div>
        </div>
      </div>
      ...
      <div className="border-t border-white/5 pt-5">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-4">
          ...
          {!neutralVenue && <Slider label="Home advantage" value={homeAdv} min={1.0} max={1.3} step={0.01} onChange={setHomeAdv} format={(v) => `×${v.toFixed(2)}`} />}
          <div className="flex items-center gap-3 pt-1">
            <button onClick={() => setNeutralVenue(!neutralVenue)} className={`w-9 h-5 rounded-full transition-colors flex-shrink-0 relative ${neutralVenue ? 'bg-wcGreen' : 'bg-gray-700'}`}>
      ...
      <div className="border-t border-white/5 pt-5">
        ...
        <div className="overflow-x-auto">
          <table className="text-center text-xs w-full min-w-[320px]">
            <thead>
              <tr>
                <td className="pb-2 pr-2 text-gray-600 text-right"><span className="text-wcGreen">{homeTeam} →</span></td>
                {Array.from({ length: MAX_GOALS + 1 }, (_, a) => <td key={a} className="pb-2 w-10 font-bold text-gray-400">{a}</td>)}
              </tr>
            </thead>
            <tbody>
              {Array.from({ length: MAX_GOALS + 1 }, (_, h) => (
                <tr key={h}>
                  ...
                  {Array.from({ length: MAX_GOALS + 1 }, (_, a) => {
                    const p = grid[h][a];
                    return (
                      <td key={a} className="py-0.5 w-10 h-8 rounded text-xs font-semibold tabular-nums"
          ...
          <div className="flex items-center gap-4 mt-2 text-xs text-gray-600"><span>Rows = {homeTeam} goals · Columns = {awayTeam} goals</span></div>
        </div>
      </div>
      <div className="border-t border-white/5 pt-5">
            ...
            return (
              <div key={`${h}-${a}`} className="flex items-center gap-3">
                ...
                <div className="flex-1 h-1.5 bg-gray-800 rounded-full overflow-hidden">
                  <motion.div className={`h-full rounded-full ${h > a ? 'bg-wcGreen' : h < a ? 'bg-wcRed' : 'bg-gray-500'}`}
                    animate={{ width: `${(p / maxScorelineP) * 100}%` }} transition={{ type: 'spring', stiffness: 120, damping: 20 }} />
                </div>
