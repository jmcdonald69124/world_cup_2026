...
function CacheStatus() {
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => { setStats(cache.stats()); setQuota(getDailyQuotaRemaining()); }, 10000);
    return () => clearInterval(id);
  }, []);
  if (stats.entries === 0 && quota === null) return null;
  return (
    <div title={`Cache: ${stats.entries} entries, ${stats.sizeKb} KB\nAPI-Football: ${quota ?? '?'} requests remaining today`}
      ...
      {quota !== null && (<><span className="text-gray-600">·</span><span className={`text-[10px] font-mono ${quota < 20 ? 'text-orange-400' : 'text-gray-400'}`}>{quota}/100 API</span></>)}
    </div>
  );
}
...
export default function Navbar() {
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? 'bg-wcDark/95 backdrop-blur-xl shadow-lg shadow-black/50' : 'bg-transparent'}`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          <div className="hidden md:flex items-center gap-1">
            {navLinks.map((link) => (
              <NavLink key={link.to} to={link.to}
                className={({ isActive }) => `px-4 py-2 rounded-lg text-sm font-medium transition-all duration-200 ${isActive ? 'bg-wcGreen/20 text-wcGreen' : 'text-gray-400 hover:text-white hover:bg-white/5'}`}>
                {link.label}
              </NavLink>
            ))}
          </div>
          ...
          <button onClick={() => setMenuOpen(!menuOpen)} className="md:hidden p-2 rounded-lg text-gray-400 hover:text-white hover:bg-white/10 transition-colors" aria-label="Toggle menu">
        ...
        {menuOpen && (
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }}
            ...
            <div className="px-4 py-3 space-y-1">
              {navLinks.map((link) => (
                <NavLink key={link.to} to={link.to}
                  className={({ isActive }) => `block px-4 py-3 rounded-lg text-sm font-medium transition-all ${isActive ? 'bg-wcGreen/20 text-wcGreen' : 'text-gray-400 hover:text-white hover:bg-white/5'}`}>
                  {link.label}
                </NavLink>
              ))}
