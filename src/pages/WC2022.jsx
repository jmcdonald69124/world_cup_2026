    ...
    >
      <div className="flex items-start justify-between gap-3 mb-3">
        ...
        <div className="text-right flex-shrink-0">
          {isUpset && <div className="text-xs font-bold text-wcRed uppercase tracking-wider mb-1">Upset</div>}
  ...
  return (
    <div className="min-h-screen pt-20 pb-16 px-4">
      <div className="max-w-6xl mx-auto">
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }} className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-8">
          ...
          {TABS.map(tab => (
            <button key={tab} onClick={() => setActiveTab(tab)}
              className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${
                activeTab === tab ? 'bg-wcGreen/20 text-wcGreen border border-wcGreen/30' : 'text-gray-400 hover:text-white hover:bg-white/5'
              }`}>
              {tab}
            </button>
          ))}
        ...
        {activeTab === 'Overview' && (
          <div className="space-y-10">
                    ...
                    <div key={round}>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2">
                        {roundMatches.map(m => {
                          ...
                          const actual = getActualResult(m);
                          const resultStr = m.pens ? `${m.score.home}–${m.score.away} AET (${m.pens.home}–${m.pens.away} pens)` : `${m.score.home}–${m.score.away}`;
                          return (
                            <div key={m.id} className={`glass-card rounded-lg p-3 ${m.id === 'FIN' ? 'border border-wcGold/30' : 'border border-white/5'}`}>
                              ...
                              <div className="flex items-center justify-between gap-2">
                                <div className={`flex items-center gap-1 text-xs ${actual === 'home' ? 'text-white font-bold' : 'text-gray-500'}`}>
                                  <span>{home?.flagEmoji}</span><span>{home?.code}</span>
                                </div>
                                ...
                                <div className={`flex items-center gap-1 text-xs ${actual === 'away' ? 'text-white font-bold' : 'text-gray-500'}`}>
                                  <span>{away?.code}</span><span>{away?.flagEmoji}</span>
                                </div>
                              </div>
                            </div>
                          );
            ...
            <div>
              <SectionHeader title="Poisson Model Performance" subtitle="How well did ELO-based Poisson predictions match the actual results?" />
              ...
              <div className="glass-card rounded-xl p-5 border border-white/5">
                <p className="text-sm text-gray-400 leading-relaxed">
                  The Poisson model derived expected goals from pre-tournament ELO ratings, then computed outcome probabilities across all scorelines up to 10 goals. The average Brier score of <strong className="text-white">{avgBrier.toFixed(3)}</strong> indicates moderate predictive skill. The model correctly identified the favourite in <strong className="text-white">{Math.round((correctPredictions / total) * 100)}%</strong> of matches.
                </p>
              </div>
            </div>
          </div>
        ...
        {activeTab === 'Group Standings' && (
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {WC2022_GROUPS.map(g => <GroupStandings key={g} group={g} />)}
            </div>
          </div>
        )}
        ...
        {activeTab === 'Model vs Reality' && (
          <div>
            <SectionHeader title="Model vs Reality" subtitle="Poisson predictions (from pre-match ELO) compared to actual results" />
            <div className="flex flex-wrap gap-1.5 mb-6">
              {rounds.map(r => (
                <button key={r} onClick={() => setSelectedRound(r)}
                  className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${
                    selectedRound === r ? 'bg-wcBlue/30 text-white border border-wcBlue/50' : 'bg-white/5 text-gray-500 hover:text-white'
                  }`}>
                  {r}
                </button>
              ))}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              {filteredMatches.map((match, i) => <ModelVsRealityRow key={match.id} match={match} rank={i} />)}
            </div>
          </div>
        )}
        ...
        {activeTab === 'Biggest Upsets' && (
          <div>
            <SectionHeader title="Biggest Upsets" subtitle="Matches where the actual winner had the lowest model-assigned probability" />
            <div className="mb-6 glass-card rounded-xl p-5 border border-white/5">
              <p className="text-sm text-gray-400 leading-relaxed">An upset is defined here as a result the Poisson model assigned a low probability. The model assigns probabilities based solely on ELO differentials — it cannot account for tactical surprises, injuries, or the psychological weight of a single tournament match.</p>
            </div>
            <div className="space-y-4">
              {biggestUpsets.map((m, i) => {
                ...
                return (
                  <motion.div key={m.id} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.1 }} className="glass-card rounded-xl p-5 border border-wcRed/20">
                    <div className="flex items-start justify-between gap-4">
                      <div>
                        ...
                        <div className="text-lg font-black text-white mb-0.5">{homeTeam?.flagEmoji} {homeTeam?.code} {resultStr} {awayTeam?.code} {awayTeam?.flagEmoji}</div>
                        {winnerTeam && loserTeam && (
                          <div className="text-sm text-gray-400">{winnerTeam.name} ({winnerTeam.eloRating} ELO) defeated {loserTeam.name} ({loserTeam.eloRating} ELO)</div>
                        )}
                      </div>
                      <div className="text-right flex-shrink-0">
                    ...
                    <div className="grid grid-cols-3 gap-2 mt-4 text-xs text-center">
                      {[{ label: homeTeam?.code, prob: m.probs.pHomeWin }, { label: 'Draw', prob: m.probs.pDraw }, { label: awayTeam?.code, prob: m.probs.pAwayWin }].map(({ label, prob }) => (
                        <div key={label} className="bg-white/5 rounded-lg py-2"><div className="text-gray-500">{label}</div><div className="font-bold text-white tabular-nums">{(prob * 100).toFixed(0)}%</div></div>
                      ))}
                    </div>
                    <div className="mt-3 h-1 bg-gray-800 rounded-full overflow-hidden">
                      <div className="h-full bg-wcRed rounded-full" style={{ width: `${m.actualProb * 100}%` }} />
                    </div>
          ...
          <div>
            ...
            <div className="space-y-2 mb-8">
              {WC2022_TOP_SCORERS.map((scorer, i) => {
                ...
                return (
                  <motion.div key={scorer.name} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.07 }} className="glass-card rounded-xl p-4 flex items-center gap-4">
                    ...
                    <div className="text-right"><div className="text-xl font-black text-wcGold tabular-nums">{scorer.goals}</div><div className="text-xs text-gray-600">goals</div></div>
                    {scorer.assists > 0 && <div className="text-right w-12"><div className="text-base font-bold text-gray-400 tabular-nums">{scorer.assists}</div><div className="text-xs text-gray-600">ast</div></div>}
                    <div className="w-24 hidden sm:block"><div className="h-1.5 bg-gray-800 rounded-full overflow-hidden"><div className="h-full bg-wcGold rounded-full" style={{ width: `${(scorer.goals / maxGoals) * 100}%` }} /></div></div>
                  </motion.div>
                );
              })}
            </div>
