  ...
  useEffect(() => {
    ...
    if (currentMinute > 0) {
      g.append('line')
        ...
        .transition()
        .duration(800)
        .attr('x2', xScale(currentMinute));

      g.append('circle')
        .attr('cx', xScale(currentMinute))
        .attr('cy', centerY)
        .attr('r', 5)
        .attr('fill', '#00A550')
        .attr('opacity', 0)
    ...
    g.append('text')
      ...
      .attr('font-size', '10px')
      .attr('font-weight', 'bold')
      .text(homeTeam.length > 10 ? homeTeam.substring(0, 10) + '…' : homeTeam);
    ...
    g.append('text')
      ...
      .attr('font-size', '10px')
      .attr('font-weight', 'bold')
      .text(awayTeam.length > 10 ? awayTeam.substring(0, 10) + '…' : awayTeam);
    ...
    events.forEach((event, idx) => {
      ...
      if (event.type === 'yellowCard') { eventIcon = '🟨'; eventColor = '#EAB308'; }
      if (event.type === 'redCard') { eventIcon = '🟥'; eventColor = '#EF4444'; }

      g.append('line')
        .attr('x1', ex)
        .attr('y1', centerY)
        .attr('x2', ex)
        .attr('y2', isHome ? centerY - 15 : centerY + 15)
        .attr('stroke', eventColor)
        .attr('stroke-width', 1.5)
        .attr('opacity', 0)
        .transition().delay(idx * 100 + 300).duration(300)
        .attr('opacity', 0.7);

      const evtCircle = g.append('circle')
        .attr('cx', ex)
        .attr('cy', ey)
        .attr('r', 12)
        .attr('fill', '#1f2937')
      ...
      evtCircle
        .on('mouseover', function (e) {
          const tooltipText = event.type === 'goal'
            ? `⚽ ${event.player || 'Goal'} ${event.minute}' ${event.assist ? `(assist: ${event.assist})` : ''}`
            : event.type === 'yellowCard'
              ? `🟨 Yellow: ${event.player} ${event.minute}'`
              : `🟥 Red: ${event.player} ${event.minute}'`;
