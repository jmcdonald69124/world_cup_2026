...
function normaliseFixtures(fdMatches) {
  return fdMatches.map((m, i) => ({
    ...
    homeTeam: {
      ...
      flagEmoji: '🏳', // fd doesn't provide emojis — keep as placeholder
      eloRating: 1800,
    },
    awayTeam: {
...
/** Returns cached live schedule (array) or null if still on demo data. */
...
/** True once real 2026 data has been fetched at least once. */
export function hasLiveData() {
  return Boolean(localStorage.getItem(STORAGE_KEY));
}

/** Clears cached live data (useful during development). */
 ...
 * Silently no-ops if already polled today or if no API key is set.
 */
export async function pollForLiveData() {
  ...
  try {
    ...
    if (!res.ok) {
      // 404 = tournament not yet in API; 403 = bad key — both are expected pre-2026
    ...
    // Notify the app so components can switch to live data without a reload
